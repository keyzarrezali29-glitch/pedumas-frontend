import { Camera, Save, ShieldCheck, User2, LockKeyhole, Mail, Crown } from "lucide-react"
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:24, fontFamily:"'Plus Jakarta Sans',sans-serif" }}>
      <style>{`
        ...
        .spfield:focus { border-color: #B22222 !important; box-shadow: 0 0 0 3px rgba(178,34,34,.1); }
      `}</style>
        ...
        <div style={{ position:"relative", zIndex:10, display:"flex", alignItems:"center", justifyContent:"space-between" }}>
          <div>
            <div style={{ display:"inline-flex", alignItems:"center", gap:8, background:"rgba(255,255,255,.2)", padding:"6px 14px", borderRadius:100, fontSize:12, fontWeight:700, marginBottom:16 }}>
              <Crown size={13} /> SUPER ADMIN PROFILE
            </div>
            <h1 style={{ fontSize:36, fontWeight:800, marginBottom:8 }}>Profile Super Admin</h1>
            <p style={{ color:"rgba(255,255,255,.75)", fontSize:15, lineHeight:1.7 }}>Kelola informasi dan keamanan akun Super Admin PEDUMAS.</p>
          </div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 2fr", gap:24 }}>
        ...
        <div style={{ background:"white", borderRadius:24, padding:28, border:`1.5px solid ${R.light}`, boxShadow:"0 4px 20px rgba(0,0,0,.05)", alignSelf:"start" }}>
          <div style={{ display:"flex", flexDirection:"column", alignItems:"center", textAlign:"center" }}>
            ...
            <span style={{ background:"#EDE9FE", color:"#7C3AED", padding:"6px 16px", borderRadius:100, fontSize:12, fontWeight:700 }}>SUPER ADMIN</span>
            <div style={{ width:"100%", marginTop:24, display:"flex", flexDirection:"column", gap:10 }}>
              {[
                { label:"Role", value:"Super Admin" },
                { label:"Akses", value:"Full Access" },
                { label:"Status", value:"Active" },
              ].map((item,i) => (
                <div key={i} style={{ background:R.light, borderRadius:12, padding:"11px 16px", display:"flex", justifyContent:"space-between" }}>
                  <span style={{ fontSize:13, color:R.muted }}>{item.label}</span>
                  <span style={{ fontSize:13, fontWeight:700, color:item.label==="Status"?"#059669":item.label==="Akses"?"#7C3AED":R.main }}>{item.value}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <div style={{ background:"white", borderRadius:24, padding:28, border:`1.5px solid ${R.light}`, boxShadow:"0 4px 20px rgba(0,0,0,.05)" }}>
          ...
          <p style={{ fontSize:14, color:R.muted, marginBottom:28 }}>Update informasi akun Super Admin kamu.</p>

          <form onSubmit={handleUpdate} style={{ display:"flex", flexDirection:"column", gap:20 }}>
            <div>
              <label style={{ display:"flex", alignItems:"center", gap:8, fontWeight:600, fontSize:14, color:R.text, marginBottom:8 }}>
                <User2 size={16} color={R.main} /> Nama Lengkap
              </label>
              <input className="spfield" style={inputStyle} type="text" value={name} onChange={e => setName(e.target.value)} required />
            </div>
            <div>
              <label style={{ display:"flex", alignItems:"center", gap:8, fontWeight:600, fontSize:14, color:R.text, marginBottom:8 }}>
                <Mail size={16} color={R.main} /> Email
              </label>
              <input style={{...inputStyle, background:"#f9f9f9", color:"#9ca3af", cursor:"not-allowed"}} type="email" disabled value={user.email} />
            </div>
            <div>
              <label style={{ display:"flex", alignItems:"center", gap:8, fontWeight:600, fontSize:14, color:R.text, marginBottom:8 }}>
                <LockKeyhole size={16} color={R.main} /> Password Baru
              </label>
              <input className="spfield" style={inputStyle} type="password" placeholder="Kosongkan jika tidak ingin mengganti" value={password} onChange={e => setPassword(e.target.value)} />
            </div>
            <div>
              <label style={{ display:"flex", alignItems:"center", gap:8, fontWeight:600, fontSize:14, color:R.text, marginBottom:8 }}>
                <Camera size={16} color={R.main} /> Upload Avatar
              </label>
